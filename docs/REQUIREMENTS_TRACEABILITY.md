...
| Controlled Transparency, Commercial Confidentiality & Authorization Foundation | `apps.governance` (new app); `apps.procurement` (ProcurementPackage + 6 commercial-layer objects); `apps.audit` (EvidenceBundle/EvidenceItem) (ADR-041) | `Party`, `PartyMembership`, `RoleAssignment`, `CapabilityGrant`, `DisclosureGrant`, `ChangeRequest`, `RiskFlag`, `DerivedArtifact`; `ProcurementPackage`, `FactoryRFQ`, `InternalCommercialSheet`, `ClientQuote`, `VerificationAssertion`; `EvidenceBundle`, `EvidenceItem` | `/compras/paquetes/` package detail (role-based projection), `/gobernanza/` Party/privileged-audit admin | `tests/test_governance.py` (10), `tests/test_procurement_confidentiality.py` (11), `tests/test_evidence_and_disclosure.py` (13), `tests/test_derived_artifacts.py` (12), `tests/test_workflow_template_preservation.py` (3), `tests/test_confidentiality_http.py` (12) — 61 tests | Done — live-validated end-to-end with a real DT Beach buyer, China Trading Co, Edison as China procurement operator, a hidden factory, and a DT Beach client user; the client saw only the approved quote and a client-safe verification statement while every confidential section (factory identity, address, cost, markup, margin) was completely absent from both the page and the API; partial Disclosure Grant, revocation, package freeze/change-request/hold, and independent-verifier evidence-bundle verification all confirmed live; see `docs/CONTROLLED_TRANSPARENCY_AND_CONFIDENTIALITY.md` |
