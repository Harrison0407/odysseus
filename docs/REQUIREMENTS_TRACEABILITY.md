...
| Lawson training and reference-installation sessions | `apps.training` (new app) | `TrainingSession`, `TrainingChecklistItem`, `TrainingParticipantAcknowledgement`, `TrainingEvidence`, `TrainingCategory` (ADR-037); `FieldIssue.training_session` (extended) | `/capacitaciones/` create/list/detail | `tests/test_training_sessions.py` (18 tests) | Done — live-validated in the real ARENA T1 Building 11 |
...
| Controlled Transparency, Commercial Confidentiality & Authorization Foundation | `apps.governance`; `apps.procurement` package/commercial layers; `apps.audit` EvidenceBundle/EvidenceItem (ADR-041, ADR-042, ADR-043) | `Party`, `PartyMembership`, `RoleAssignment`, `CapabilityGrant`, `DisclosureGrant`, `ChangeRequest`, `RiskFlag`, `DerivedArtifact`; `ProcurementPackage`, commercial-layer records, `VerificationAssertion`; `EvidenceBundle`, `EvidenceItem` | Package list/detail and field-scoped disclosure projection; retrieval-safe, completeness-correct scoped privileged audit (CTCF-AUDIT-017, CTCF-AUDIT-SCOPE-021, CTCF-AUDIT-RETRIEVAL-022, CTCF-AUDIT-WINDOW-023); authorized Change Request projection (CTCF-CR-PROJ-018); package-scoped affected APIs | Original 61-test foundation suite plus `tests/test_foundation_remediation.py` (17 adversarial tests) plus `tests/test_privileged_audit_scope.py` (31 tests: cycle 2's 16 authority/scope/information-absence tests plus cycle 3's 15 `TestCapabilityGrantScopeInheritance`/`TestRetrievalBeforeAuthorization`/`TestScanWindowCompleteness` tests) and `tests/test_change_request_projection.py` (8 tests) | Done; remediation implemented and locally integration-verified — same/different-organization and cross-package denial, document-byte isolation, target-derived evidence authorization, live disclosure expiry/revocation, draft/expired/revoked information absence, Change Request holds, quote separation of duties, risk authorization, affected API scoping, scoped privileged-audit access (organization/package-scoped `VIEW_PRIVILEGED_AUDIT` including role_assignment-inherited scope, cross-tenant and cross-package isolation, fail-closed unresolvable targets, SQL-verified retrieval-before-authorization with no `summary`/`metadata` selected at any phase, and cursor-paginated scan completeness beyond any fixed candidate-window size), and field-specific Change Request read authorization all pass. Production-runtime and independent revalidation of foundation correction cycle 3 remain outstanding; no raw evidence projection or A1–A6 gates were added. `VerificationAssertion` revocation remains service-only (CTCF-ASSERT-HTTP-019, deferred by explicit decision). |
